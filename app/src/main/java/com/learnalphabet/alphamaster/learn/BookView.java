...
public class BookView extends GLSurfaceView implements View.OnTouchListener,
		BookRenderer.Observer {
	...
	// show page flipping.
	private BookMesh mPageCurl;

	private BookMesh mPageLeft;
	private PageProvider mPageProvider;
	private BookMesh mPageRight;
	 ...
	 * Default constructor.
	 */
	public BookView(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
		init(ctx);
	}
	 ...
	 * Default constructor.
	 */
	public BookView(Context ctx, AttributeSet attrs, int defStyle) {
		this(ctx, attrs);
	}
	...
	private void init(Context ctx) {
		mRenderer = new BookRenderer(this);
		...
		mPageLeft = new BookMesh(10);
		mPageRight = new BookMesh(10);
		mPageCurl = new BookMesh(10);
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
				...
				BookMesh right = mPageCurl;
				BookMesh curl = mPageRight;
				right.setRect(mRenderer.getPageRect(BookRenderer.PAGE_RIGHT));
				...
				BookMesh left = mPageCurl;
				BookMesh curl = mPageLeft;
				left.setRect(mRenderer.getPageRect(BookRenderer.PAGE_LEFT));
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		RectF rightRect = mRenderer.getPageRect(BookRenderer.PAGE_RIGHT);
		RectF leftRect = mRenderer.getPageRect(BookRenderer.PAGE_LEFT);
			...
			if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
				...
				if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2)
					...
					mAnimationTarget.x = mRenderer
							.getPageRect(BookRenderer.PAGE_RIGHT).right;
	...
	private void setCurlPos(PointF curlPos, PointF curlDir, double radius) {
		...
		if (mCurlState == CURL_RIGHT
			...
			RectF pageRect = mRenderer.getPageRect(BookRenderer.PAGE_RIGHT);
			...
			RectF pageRect = mRenderer.getPageRect(BookRenderer.PAGE_LEFT);
			if (curlPos.x <= pageRect.left) {
				mPageCurl.reset();
				requestRender();
				return;
	...
	public void setViewMode(int viewMode) {
		...
		case SHOW_ONE_PAGE:
			mViewMode = viewMode;
			mPageLeft.setFlipTexture(true);
			mRenderer.setViewMode(BookRenderer.SHOW_ONE_PAGE);
			break;
		case SHOW_TWO_PAGES:
			mViewMode = viewMode;
			mPageLeft.setFlipTexture(false);
			mRenderer.setViewMode(BookRenderer.SHOW_TWO_PAGES);
			break;
		}
	}
	...
	private void startCurl(int page) {
		...
		case CURL_RIGHT: {
			...
			if (mCurrentIndex > 0) {
				mPageLeft.setFlipTexture(true);
				mPageLeft
						.setRect(mRenderer.getPageRect(BookRenderer.PAGE_LEFT));
				mPageLeft.reset();
				if (mRenderLeftPage) {
			...
			if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
				...
				mPageRight.setRect(mRenderer
						.getPageRect(BookRenderer.PAGE_RIGHT));
				mPageRight.setFlipTexture(false);
				mPageRight.reset();
			...
			mPageCurl.setRect(mRenderer.getPageRect(BookRenderer.PAGE_RIGHT));
	...
	private void updateCurlPos(PointerPosition pointerPos) {
		...
		if (mCurlState == CURL_RIGHT
			...
			float pageWidth = mRenderer.getPageRect(BookRenderer.PAGE_RIGHT)
					.width();
			...
			if (dist >= curlLen) {
				...
				if (mViewMode == SHOW_TWO_PAGES) {
					...
					float pageLeftX = mRenderer
							.getPageRect(BookRenderer.PAGE_RIGHT).left;
		...
		// Otherwise we'll let curl follow pointer position.
			...
			float pageLeftX = mRenderer.getPageRect(BookRenderer.PAGE_RIGHT).left;
			radius = Math.max(Math.min(mCurlPos.x - pageLeftX, radius), 0f);

			float pageRightX = mRenderer.getPageRect(BookRenderer.PAGE_RIGHT).right;
	 ...
	 */
	private void updatePage(BookPage page, int index) {
	...
	private void updatePages() {
		...
		if (leftIdx >= 0 && leftIdx < mPageProvider.getPageCount()) {
			//Log.d(getClass().getName(),"leftIdx="+leftIdx);
			updatePage(mPageLeft.getTexturePage(), leftIdx);
			mPageLeft.setFlipTexture(true);
			mPageLeft.setRect(mRenderer.getPageRect(BookRenderer.PAGE_LEFT));
			mPageLeft.reset();
			if (mRenderLeftPage) {
				mRenderer.addCurlMesh(mPageLeft);
			}
		...
		if (rightIdx >= 0 && rightIdx < mPageProvider.getPageCount()) {
			//Log.d(getClass().getName(),"rightIdx="+rightIdx);
			updatePage(mPageRight.getTexturePage(), rightIdx);
			mPageRight.setFlipTexture(false);
			mPageRight.setRect(mRenderer.getPageRect(BookRenderer.PAGE_RIGHT));
			mPageRight.reset();
			mRenderer.addCurlMesh(mPageRight);
		}
