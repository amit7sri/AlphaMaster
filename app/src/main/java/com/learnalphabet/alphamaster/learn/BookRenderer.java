...
public class BookRenderer implements GLSurfaceView.Renderer {
	...
	private Vector<BookMesh> mBookMeshs;
	private RectF mMargins = new RectF();
	private BookRenderer.Observer mObserver;
	// Page rectangles.
	 ...
	 * Basic constructor.
	 */
	public BookRenderer(BookRenderer.Observer observer) {
		mObserver = observer;
		mBookMeshs = new Vector<BookMesh>();
	 ...
	 */
	public synchronized void addCurlMesh(BookMesh mesh) {
		removeCurlMesh(mesh);
		mBookMeshs.add(mesh);
	}
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		for (int i = 0; i < mBookMeshs.size(); ++i) {
			mBookMeshs.get(i).onDrawFrame(gl);
		}
	}
	 ...
	 */
	public synchronized void removeCurlMesh(BookMesh mesh) {
		while (mBookMeshs.remove(mesh))
			;
	}
