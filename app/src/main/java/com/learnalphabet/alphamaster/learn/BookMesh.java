...
public class BookMesh {
	...
	private final BookPage mTexturePage = new BookPage();
	 ...
	 */
	public BookMesh(int maxCurlSplits) {
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			while (mArrOutputVertices.size() > 0) {
				...
				if (textureFront != mFlipTexture) {
					...
					v.mColor = mTexturePage.getColor(BookPage.SIDE_FRONT);
				} else {
					v.mTexX *= mTextureRectBack.right;
					v.mTexY *= mTextureRectBack.bottom;
					v.mColor = mTexturePage.getColor(BookPage.SIDE_BACK);
				}
	 ...
	 */
	public synchronized BookPage getTexturePage() {
		return mTexturePage;
	}
	...
	public synchronized void reset() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (mFlipTexture) {
				...
				tmp.mColor = mTexturePage.getColor(BookPage.SIDE_BACK);
			} else {
				tmp.mTexX *= mTextureRectFront.right;
				tmp.mTexY *= mTextureRectFront.bottom;
				tmp.mColor = mTexturePage.getColor(BookPage.SIDE_FRONT);
			}
